...
public class DevicesController {
    ...
    public ResponseEntity<Map<String, Object>> page(String key_word,
        ...
        //添加过滤条件，使用like关键字
        queryWrapper.like(key_word != null, Devices::getName, key_word);

        //添加排序条件
        queryWrapper.orderByDesc(Devices::getDid);

        //执行分页查询
        devicesService.page(pageInfo, queryWrapper);
     ...
     */
    @PostMapping("/add")
    public Result<String> add (Devices devices)  {
    ...
    public Result<String> delete(String did) {
        if (did == null){
            return Result.error("delete error");
        } else {
            devicesService.removeById(did);
        }
