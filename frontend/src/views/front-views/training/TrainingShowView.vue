...
const update = (trainingId) => {
  const result = localStorage.getItem(`${trainingId}`) || ''
  if(result){
...
onBeforeRouteUpdate((to, from, next) => {
  // 当路由参数变化且当前组件被复用时执行
  if (to.query.trainingId !== from.query.trainingId) {
    update(to.query.trainingId)
  }
  next();
});

onMounted(() =>{
  update(route.query.trainingId)
})
</script>
